...
router.post("/create", passport.parseForm, passport.csrfProtection, (req, res) => {
  companies.create(req.body, (err, id) => {
    if (err || !id) {
      req.flash("danger", (err && err.message) || err || _("Could not create company"));
...
router.post("/ajax", (req, res) => {
  // return res.json({});
  console.log("📄 companies.js:: > req.body, req.query", JSON.stringify(req.body), JSON.stringify(req.query));

  companies.filter(req.body, Number(req.query.parent) || false, (err, data, total, filteredTotal) => {
    if (err) {
      return res.json({
    ...
    data.map((row, i) => {
      console.log("📄 companies.js::data.map > (row)", row);
    });
    res.json({
      draw: req.body.draw,
      recordsTotal: total,
      ...
      data: data.map((row, i) => [
        ...
        '<span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span> <a href="/companies/view/' +
          row.id +
          '">' +
          htmlescape(row.name || "") +
          "</a>",
        ...
        '<span class="glyphicon glyphicon-wrench" aria-hidden="true"></span><a href="/companies/edit/' + row.id + '">' + _("Edit") + "</a>",
      ]),
    });
  });
});
...
router.get("/view/:id", passport.csrfProtection, (req, res) => {
  companies.get(req.params.id, (err, company) => {
    if (err || !company) {
      req.flash("danger", (err && err.message) || err || _("Could not find company with specified ID"));
...
router.get("/edit/:id", passport.csrfProtection, (req, res) => {
  companies.get(req.params.id, (err, company) => {
    ...
    //   company.unsubscriptionModeOptions = getUnsubscriptionModeOptions(company.unsubscriptionMode);
...
router.post("/edit", passport.parseForm, passport.csrfProtection, (req, res) => {
  companies.update(req.body.id, req.body, (err, updated) => {
    ...
    if (req.query.next) {
      ...
      return res.redirect("/companies/edit/" + encodeURIComponent(req.body.id));
    } else {
      return res.redirect("/companies");
    }
  });
...
//   companies.get(req.params.id, (err, list) => {
//     if (err || !list) {
//       return res.json({
...
//     fields.list(company.id, (err, fieldCompany) => {
//       if (err && !fieldCompany) {
//         fieldCompany = [];
//       }
...
//         (err && err.message) ||
//           err ||
//           _("Could not find list with specified ID")
//       );
//       return res.redirect("/companies");
//     }

//     subscriptions.listImports(company.id, (err, imports) => {
//       if (err) {
...
//         res.render("companies/view", list);
//       });
//     });
//   });
// });
...
//             encodeURIComponent(req.body.list) +
//             "/add?" +
...
//           "success",
//           util.format(
//             _("%s was successfully added to your list"),
//             req.body.email
//           )
//         );
//       } else {
//         req.flash(
//           "warning",
//           util.format(_("%s was not added to your list"), req.body.email)
//         );
//       }

//       res.redirect(
//         "/companies/subscription/" + encodeURIComponent(req.body.list) + "/add"
//       );
//     });
//   }
// );
...
//   passport.csrfProtection,
//   (req, res) => {
//     companies.get(req.body.list, (err, list) => {
//       if (err || !list) {
//         req.flash(
//           "danger",
//           (err && err.message) ||
...
//               );
//               return res.redirect(
//                 "/companies/subscription/" + company.id + "/edit/" + subscription.cid
//               );
//             }
//             req.flash(
//               "success",
//               util.format(
//                 _("%s was successfully unsubscribed from your list"),
//                 subscription.email
//               )
//             );
...
//         req.flash(
//           "success",
//           util.format(_("%s was successfully removed from your list"), email)
//         );
//         res.redirect("/companies/view/" + company.id);
//       });
//     });
...
//         if (req.body.list) {
//           return res.redirect(
//             "/companies/view/" + encodeURIComponent(req.body.list)
//           );
//         } else {
//           return res.redirect("/companies");
//         }
...
// router.get(
//   "/subscription/:id/import/:importId",
//   passport.csrfProtection,
//   (req, res) => {
//     companies.get(req.params.id, (err, list) => {
//       if (err || !list) {
...
//   companies.filterQuicklist(req.body, (err, data, total, filteredTotal) => {
//     if (err) {
//       return res.json({
//         error: err.message || err,
//         data: [],
