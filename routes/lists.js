...
router.get("/", (req, res) => {
  console.log("📄 lists.js::router.get(/) > ");
...
router.get("/create", passport.csrfProtection, (req, res) => {
  ...
  data.unsubscriptionModeOptions = getUnsubscriptionModeOptions(
    data.unsubscriptionMode || lists.UnsubscriptionMode.ONE_STEP
  );
  ...
  (req, res) => {
    lists.create(req.body, (err, id) => {
      if (err || !id) {
        req.flash(
          "danger",
          (err && err.message) || err || _("Could not create list")
        );
...
router.get("/edit/:id", passport.csrfProtection, (req, res) => {
  lists.get(req.params.id, (err, list) => {
    ...
    forms.list(list.id, (err, customForms) => {
      ...
      list.unsubscriptionModeOptions = getUnsubscriptionModeOptions(
        list.unsubscriptionMode
      );
    ...
    (err, data, total, filteredTotal) => {
      ...
      res.json({
        ...
        data: data.map((row, i) => [
          ...
          '<span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span> <a href="/lists/view/' +
            row.id +
            '">' +
            htmlescape(row.name || "") +
            "</a>",
          ...
          '<span class="glyphicon glyphicon-wrench" aria-hidden="true"></span><a href="/lists/edit/' +
            row.id +
            '">' +
            _("Edit") +
            "</a>",
          ...
          res.json({
            ...
            data: data.map((row, i) =>
              [
                ...
                .concat(
                  fields.getRow(fieldList, row).map((cRow) => {
                    if (cRow.type === "number") {
                      return htmlescape(
  ...
  lists.get(req.params.id, (err, list) => {
    ...
    subscriptions.listImports(list.id, (err, imports) => {
      ...
      fields.list(list.id, (err, fieldList) => {
        ...
        list.customSort = list.customFields.length
          ? "," + list.customFields.map(() => "0").join(",")
          : "";

        list.showSubscriptions =
...
router.post(
  ...
  (req, res) => {
    subscriptions.insert(req.body.list, false, req.body, (err, response) => {
      ...
      res.redirect(
        "/lists/subscription/" + encodeURIComponent(req.body.list) + "/add"
      );
    });
  }
);
        ...
        subscriptions.changeStatus(
          ...
          (err, found) => {
            if (err) {
              ...
              return res.redirect(
                "/lists/subscription/" + list.id + "/edit/" + subscription.cid
              );
            }
            req.flash(
              "success",
  ...
  (req, res) => {
    ...
    subscriptions.update(
      ...
      (err, updated) => {
        ...
        if (req.body.list) {
          return res.redirect(
            "/lists/view/" + encodeURIComponent(req.body.list)
          );
        } else {
          return res.redirect("/lists");
        }
        ...
        if (err) {
          ...
          subscriptions.createImport(
            ...
            (err, importId) => {
              ...
              return res.redirect(
                "/lists/subscription/" + list.id + "/import/" + importId
              );
            }
          );
        }
  ...
  lists.filterQuicklist(req.body, (err, data, total, filteredTotal) => {
    ...
    res.json({
      ...
      data: data.map((row, i) => ({
        ...
        1:
          '<span class="glyphicon glyphicon-inbox" aria-hidden="true"></span> <a href="/lists/view/' +
          row.id +
          '">' +
          htmlescape(row.name || "") +
          "</a>",
...
function getUnsubscriptionModeOptions(unsubscriptionMode) {
  ...
  options[lists.UnsubscriptionMode.ONE_STEP_WITH_FORM] = {
    ...
    selected:
      unsubscriptionMode === lists.UnsubscriptionMode.ONE_STEP_WITH_FORM,
    label: _(
  ...
  options[lists.UnsubscriptionMode.TWO_STEP_WITH_FORM] = {
    ...
    selected:
      unsubscriptionMode === lists.UnsubscriptionMode.TWO_STEP_WITH_FORM,
    label: _(
