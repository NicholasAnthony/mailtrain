...
const handleErrorResponse = (res, log, err, code = 500, message = false) => {
  if (typeof err != "undefined") log.error("API", err);
  res.status(code);
  return res.json({
...
router.post("/subscribe/:listId", (req, res) => {
  ...
  Object.keys(req.body).forEach((key) => {
    input[(key || "").toString().trim().toUpperCase()] = (req.body[key] || "")
      .toString()
      .trim();
  });
        ...
        fieldList.forEach((field) => {
          if (input.hasOwnProperty(field.key) && field.column) {
            ...
            for (let i = 0, len = field.options.length; i < len; i++) {
              if (
                input.hasOwnProperty(field.options[i].key) &&
                field.options[i].column
              ) {
        ...
        if (/^(yes|true|1)$/i.test(input.REQUIRE_CONFIRMATION)) {
          ...
          confirmations.addConfirmation(
            ...
            (err, confirmCid) => {
              ...
              mailHelpers.sendConfirmSubscription(
                list,
                input.EMAIL,
                confirmCid,
                subscription,
    ...
    subscriptions.getByEmail(list.id, input.EMAIL, (err, subscription) => {
      ...
      if (!subscription) {
        return handleErrorResponse(
          ...
          false,
          404,
          "Subscription with given email not found"
        );
      }
...
router.get("/companies", (req, res) => {
  companies.quicklist((err, companies) => {
    if (err) {
      return handleErrorResponse(res, log, err);
    }
    res.status(200);
...
router.post("/field/:listId", (req, res) => {
  ...
  lists.getByCid(req.params.listId, (err, list) => {
    ...
    let field = {
      ...
      groupTemplate: (input.GROUP_TEMPLATE || "")
        .toString()
        .toLowerCase()
        .trim(),
      visible:
        ["false", "no", "0", ""].indexOf(
          (input.VISIBLE || "").toString().toLowerCase().trim()
        ) < 0,
    };
      ...
      subscriptions.getByEmail(list.id, input.EMAILOLD, (err, subscription) => {
        ...
        if (!subscription) {
          return handleErrorResponse(
            ...
            false,
            404,
            "Subscription with given old email not found"
          );
        }

        subscriptions.updateAddressCheck(
          list,
          subscription.cid,
          input.EMAILNEW,
          null,
