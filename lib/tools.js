...
function validateEmail(address, checkBlocked, callback) {
  ...
  if (checkBlocked && blockedUsers.indexOf(user) >= 0) {
    return callback(
      new Error(util.format(_('Blocked email address "%s"'), address))
    );
  }
...
function getMessageLinks(serviceUrl, campaign, list, subscription) {
  return {
    LINK_UNSUBSCRIBE: campaign.unsubscribe
    ...
    LINK_PREFERENCES: urllib.resolve(
      serviceUrl,
      "/subscription/" + list.cid + "/manage/" + subscription.cid
    ),
    LINK_BROWSER: urllib.resolve(
      serviceUrl,
      "/archive/" + campaign.cid + "/" + list.cid + "/" + subscription.cid
    ),
...
function formatMessage(
  ...
  let getTagValue = (key) => {
    ...
    if (subscription.mergeTags.hasOwnProperty(key)) {
      ...
      return isHTML
        ? he.encode(
            containsHTML ? value : value.replace(/(?:\r\n|\r|\n)/g, "<br/>"),
            {
