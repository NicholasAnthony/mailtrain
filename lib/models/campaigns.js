...
module.exports.filterClickedSubscribers = (
  campaign,
  linkId,
  request,
  columns,
  ...
  tableHelpers.filter(
    "subscription__" +
      campaign.list +
      " JOIN campaign_tracker__" +
      campaign.id +
      " ON campaign_tracker__" +
      campaign.id +
      ".subscriber=subscription__" +
      campaign.list +
      ".id",
    ["*"],
    request,
...
module.exports.statsClickedSubscribersByColumn = (
  campaign,
  linkId,
  request,
  column,
  ...
  db.getConnection((err, connection) => {
    ...
    let query_template =
      "SELECT ?? AS data, COUNT(*) AS cnt FROM `subscription__%d` JOIN `campaign_tracker__%d` ON `campaign_tracker__%d`.`list`=%d AND `campaign_tracker__%d`.`subscriber`=`subscription__%d`.`id` AND `campaign_tracker__%d`.`link`=%d  GROUP BY ?? ORDER BY COUNT(??) DESC, ??";
...
module.exports.filterStatusSubscribers = (
  campaign,
  status,
  request,
  columns,
  ...
  let queryData = {
    where:
      ...
      ".list=? AND campaign__" +
      campaign.id +
      ".segment=? AND campaign__" +
      campaign.id +
      ".status=?",
    values: [
      campaign.list,
      (campaign.segment && campaign.segment.id) || 0,
      status,
    ],
  };
...
module.exports.getByCid = (cid, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT * FROM campaigns WHERE cid=?",
      [cid],
      (err, rows) => {
        connection.release();
        if (err) {
    ...
    connection.query(
      ...
      (err, rows) => {
        ...
        db.getConnection((err, connection) => {
          ...
          connection.query(
            "SELECT `id`, `cid`, `name` FROM campaigns WHERE id=?",
            [campaign.parent],
            (err, rows) => {
              connection.release();
              if (err) {
...
module.exports.getAttachments = (campaign, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT `" +
        keys.join("`, `") +
        "` FROM `attachments` WHERE `campaign`=?",
      [campaign],
      (err, rows) => {
        connection.release();
        if (err) {
  ...
  db.getConnection((err, connection) => {
    ...
    Object.keys(attachment).forEach((key) => {
      ...
      if (Buffer.isBuffer(attachment[key])) {
        ...
        value =
          typeof attachment[key] === "number"
            ? attachment[key]
            : (attachment[key] || "").toString().trim();
      }
...
module.exports.deleteAttachment = (id, attachment, callback) => {
  db.getConnection((err, connection) => {
    ...
    let query =
      "DELETE FROM `attachments` WHERE `id`=? AND `campaign`=? LIMIT 1";
...
module.exports.getAttachment = (campaign, attachment, callback) => {
  db.getConnection((err, connection) => {
    ...
    let query =
      "SELECT * FROM `attachments` WHERE `id`=? AND `campaign`=? LIMIT 1";
...
module.exports.getLinks = (id, linkId, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    if (!linkId) {
      query =
        "SELECT `id`, `url`, `clicks` FROM links WHERE `campaign`=? LIMIT 1000";
      values = [id];
    } else {
      query =
        "SELECT `id`, `url`, `clicks` FROM links WHERE `id`=? AND `campaign`=? LIMIT 1";
      values = [linkId, id];
    }
    ...
    connection.query(query, values, (err, rows) => {
      ...
      let links = rows
        .map((row) => tools.convertKeys(row))
        .sort((a, b) =>
          a.url
            .replace(/^https?:\/\/(www.)?/, "")
            .toLowerCase()
            .localeCompare(
              b.url.replace(/^https?:\/\/(www.)?/, "").toLowerCase()
            )
        );
...
module.exports.duplicate = (id, callback) =>
  module.exports.get(id, true, (err, campaign) => {
    if (err) {
      return callback(err);
    }
    if (!campaign) {
...
module.exports.create = (campaign, opts, callback) => {
  ...
  if (
    campaign.type === 2 &&
    (!campaign.sourceUrl || !isUrl(campaign.sourceUrl))
  ) {
    return callback(
      new Error(_("RSS URL must be set and needs to be a valid URL"))
    );
  }
  ...
  getList(campaign.list, (err) => {
    ...
    let create = (next) => {
      Object.keys(campaign).forEach((key) => {
        let value =
          typeof campaign[key] === "number"
            ? campaign[key]
            : (campaign[key] || "").toString().trim();
        key = tools.toDbKey(key);
        if (key === "description") {
        ...
        mailer.getTemplate("emails/rss-html.hbs", (err, rendererHtml) => {
          ...
          create((err, campaignId) => {
            ...
            db.getConnection((err, connection) => {
              ...
              let query =
                "INSERT IGNORE INTO `rss` (`parent`,`guid`,`pubdate`) VALUES " +
                entries.map(() => "(?,?,?)").join(",");
      ...
      db.getConnection((err, connection) => {
        ...
        connection.query(
          ...
          (err, result) => {
            ...
            connection.query(
              "SELECT `type`, `source_url` FROM campaigns WHERE id=? LIMIT 1",
              [id],
              (err, rows) => {
                connection.release();
                if (err) {
...
module.exports.delete = (id, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "DELETE FROM campaigns WHERE id=? LIMIT 1",
      [id],
      (err, result) => {
        connection.release();
        if (err) {
...
module.exports.send = (id, scheduled, callback) => {
  module.exports.get(id, false, (err, campaign) => {
    ...
    db.getConnection((err, connection) => {
      ...
      if (scheduled) {
        query =
          "UPDATE campaigns SET `status`=2, `scheduled`=?, `status_change`=NOW() WHERE id=? LIMIT 1";
        values = [scheduled, id];
      } else {
        query =
          "UPDATE campaigns SET `status`=2, `status_change`=NOW() WHERE id=? LIMIT 1";
        values = [id];
      }
...
module.exports.pause = (id, callback) => {
  module.exports.get(id, false, (err, campaign) => {
    ...
    db.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE campaigns SET `status`=4, `status_change`=NOW() WHERE id=? LIMIT 1",
        [id],
        (err) => {
          connection.release();
          if (err) {
...
module.exports.reset = (id, callback) => {
  module.exports.get(id, false, (err, campaign) => {
    ...
    if (
      campaign.status !== 3 &&
      campaign.status !== 4 &&
      !(campaign.status === 2 && campaign.scheduled > new Date())
    ) {
      return callback(null, false);
    }
    ...
    db.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE campaigns SET `status`=1, `status_change`=NULL, `delivered`=0, `opened`=0, `clicks`=0, `bounced`=0, `complained`=0, `unsubscribed`=0, `blacklisted`=0 WHERE id=? LIMIT 1",
        [id],
        (err) => {
          if (err) {
            connection.release();
          ...
          db.clearCache("sender", () => {
            connection.query(
              "UPDATE links SET `clicks`=0 WHERE campaign=?",
              [id],
              (err) => {
                if (err) {
                  connection.release();
                ...
                connection.query(
                  ...
                  (err) => {
                    ...
                    connection.query(
                      "TRUNCATE TABLE `campaign_tracker__" + id + "`",
                      [id],
                      (err) => {
                        connection.release();
                        if (err) {
...
module.exports.activate = (id, callback) => {
  module.exports.get(id, false, (err, campaign) => {
    ...
    db.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE campaigns SET `status`=6, `status_change`=NOW() WHERE id=? LIMIT 1",
        [id],
        (err) => {
          connection.release();
          if (err) {
...
module.exports.inactivate = (id, callback) => {
  module.exports.get(id, false, (err, campaign) => {
    ...
    db.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE campaigns SET `status`=5, `status_change`=NOW() WHERE id=? LIMIT 1",
        [id],
        (err) => {
          connection.release();
          if (err) {
...
module.exports.getMail = (campaignId, listId, subscriptionId, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT * FROM `campaign__" +
        campaignId +
        "` WHERE list=? AND subscription=?",
...
module.exports.findMailByResponse = (responseId, callback) => {
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT id FROM campaigns ORDER BY id DESC",
      [],
      (err, campaignList) => {
        if (err) {
          connection.release();
        ...
        let checkNext = () => {
          ...
          connection.query(
            "SELECT id, list, segment, subscription FROM `campaign__" +
              campaign.id +
...
module.exports.findMailByCampaign = (campaignHeader, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    let query =
      "SELECT `id`, `list`, `segment` FROM `campaigns` WHERE `cid`=? LIMIT 1";
    connection.query(query, [cCid], (err, rows) => {
      ...
      let query =
        "SELECT id FROM `subscription__" + listId + "` WHERE cid=? LIMIT 1";
      connection.query(query, [sCid], (err, rows) => {
        ...
        let query =
          "SELECT `id`, `list`, `segment`, `subscription` FROM `campaign__" +
          campaignId +
          "` WHERE `list`=? AND `segment`=? AND `subscription`=? LIMIT 1";
        connection.query(
          query,
          [listId, segmentId, subscriptionId],
          (err, rows) => {
            connection.release();
            if (err) {
              return callback(err);
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(query, [message.campaign], () => {
      let query =
        "UPDATE `campaign__" +
        message.campaign +
        "` SET status=?, updated=NOW() WHERE id=? LIMIT 1";
...
module.exports.updateMessageResponse = (
  message,
  response,
  response_id,
  callback
  ...
  db.getConnection((err, connection) => {
    ...
    let query =
      "UPDATE `campaign__" +
      message.campaign +
      "` SET `response`=?, `response_id`=? WHERE id=? LIMIT 1";
...
function createCampaignTables(id, callback) {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(query, (err) => {
      ...
      let query =
        "CREATE TABLE `campaign_tracker__" + id + "` LIKE campaign_tracker";
      connection.query(query, (err) => {
        connection.release();
        if (err) {
          return callback(err);
