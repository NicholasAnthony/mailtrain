...
let allowedKeys = ["description", "url"];

module.exports.create = (company, callback) => {
  let data = tools.convertKeys(company);
  // data.publicSubscribe = data.publicSubscribe ? 1 : 0;

  let name = (data.name || "").toString().trim();

  if (!data) {
    return callback(new Error(_("Company Name must be set")));
  }
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(query, values, (err, result) => {
      ...
      let companyId = (result && result.insertId) || false;
      if (!companyId) {
        return callback(null, false);
      }

      createListTable(companyId, (err) => {
        if (err) {
          // FIXME: rollback
          return callback(err);
        }
...
function createListTable(id, callback) {
  let query = "CREATE TABLE `list__" + id + "` LIKE lists";
  db.getConnection((err, connection) => {
    if (err) {
      return callback(err);
    }
...
module.exports.list = (start, limit, callback) => {
  tableHelpers.list("companies", ["*"], "name", null, start, limit, callback);
};
...
module.exports.filterQuicklist = (request, callback) => {
  tableHelpers.filter(
    "companies",
    ["id", "name", "cid", "url", "desciption"],
    request,
    ["#", "name", "cid", "url", "desciption"],
    ["name"],
    "name ASC",
    null,
    callback
...
//     }
//     connection.query(
//       "SELECT id, name, description FROM companies ORDER BY name LIMIT 1000",
//       (err, rows) => {
//         if (err) {
//           connection.release();
//           return callback(err);
//         }
//         let companies = (rows || []).map(tools.convertKeys);
//         connection.query(
//           "SELECT id, list, name FROM segments ORDER BY list, name LIMIT 1000",
//           (err, rows) => {
//             connection.release();
//             if (err) {
//               return callback(err);
...
//             lists.forEach((list) => {
//               list.segments = segments.filter(
//                 (segment) => segment.list === list.id
//               );
//             });
      ...
      lists.forEach((list, index, arr) => {
        subscriptions.getByEmail(list.id, email, (err, sub) => {
          ...
          if (index === arr.length - 1) {
            return callback(
              null,
              lists.filter((list) => results.includes(list.id))
            );
          }
        });
      });
...
module.exports.get = (id, callback) => {
  id = Number(id) || 0;
  if (id < 1) {
    return callback(new Error(_("Missing Company ID")));
  }
  db.getConnection((err, connection) => {
    if (err) {
      return callback(err);
    ...
    connection.query(
      ...
      (err, rows) => {
        ...
        let company = tools.convertKeys(rows[0]);
...
module.exports.delete = (id, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "DELETE FROM companies WHERE id=? LIMIT 1",
      id,
      (err, result) => {
        connection.release();
        if (err) {
...
function resolveCid(cid, callback) {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT id FROM companies WHERE cid=?",
      [cid],
      (err, rows) => {
        connection.release();
        if (err) {
