...
module.exports.filter = (request, parent, callback) => {
  tableHelpers.filter(
    ...
    ["*"],
    request,
    ["#", "name", "cid", "subscribers", "description"],
    ["name"],
    "name ASC",
    null,
    callback
...
module.exports.quicklist = (callback) => {
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT id, name, subscribers FROM lists ORDER BY name LIMIT 1000",
      (err, rows) => {
        if (err) {
          connection.release();
          return callback(err);
...
module.exports.delete = (id, callback) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      "DELETE FROM lists WHERE id=? LIMIT 1",
      id,
      (err, result) => {
        connection.release();
        if (err) {
